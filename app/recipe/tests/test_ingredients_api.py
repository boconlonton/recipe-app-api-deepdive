...
class PrivateIngredientApiTests(TestCase):
    ...
    def test_create_ingred_success(self):
        """Test that create ingredient succesfully"""
        payload = {
            'name': 'Vinegar'
        }
        ...
        ingred_exists = Ingredient.objects.filter(
            name=payload['name'],
            user=self.user,
        ).exists()

        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.assertTrue(ingred_exists)

    def test_create_ingred_invalid(self):
        """Test that creating ingredient with invalid payload"""
        payload = {
            'name': ''
        }
